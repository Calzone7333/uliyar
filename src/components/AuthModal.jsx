...
import { X, Mail, Lock, User, Briefcase, Building, Loader, Phone, ArrowRight } from 'lucide-react';
...
const AuthModal = ({ isOpen, onClose, initialView = 'login' }) => {
    ...
    const handleRegisterSubmit = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                onClose();
                navigate(`/verify-otp?userId=${data.userId}&email=${registerData.email}`);
            } else {
                    ...
                    <div className="p-6">
                        ...
                        <div className="text-center mb-6 pt-2">
                            ...
                            <p className="text-slate-500 mt-1 text-[13px] font-medium">
                                {view === 'login' ? 'Sign in to your account' : 'Join our worker community'}
                            </p>
                        </div>
                        ...
                        {view === 'register' && (
                            <div className="flex bg-slate-50 p-1 rounded-[14px] mb-5 border border-slate-100">
                                <button
                                    type="button"
                                    onClick={() => setRegisterData({ ...registerData, role: 'employee' })}
                                    className={`flex-1 py-1.5 text-[11px] font-bold rounded-xl transition-all flex items-center justify-center gap-2 ${registerData.role === 'employee' ? 'bg-white shadow-sm text-primary border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                                >
                                    <User size={12} /> Worker
                                </button>
                                <button
                        ...
                        */}
                        ...
                        <form onSubmit={view === 'login' ? handleLoginSubmit : handleRegisterSubmit} className="space-y-3.5">
                            {view === 'register' && (
                                <>
                                    ...
                                    {/* {registerData.role === 'employer' && (
                                        <div className="relative group animate-in slide-in-from-top-1 fadeIn">
                                            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                                <Building className="h-3.5 w-3.5 text-slate-400 group-focus-within:text-primary transition-colors" />
                                            </div>
                                            <input
                                                type="text"
                                                required
                            ...
                            >
                                {isSubmitting ? <Loader size={16} className="animate-spin" /> : (
                                    view === 'login' ? <>Sign In <ArrowRight size={14} /></> : 'Verify & Join'
                                )}
                            </button>
